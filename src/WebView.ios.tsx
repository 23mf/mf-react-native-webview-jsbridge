...
class WebView extends React.Component<IOSWebViewProps, State> {
  ...
  * @param data 传递的数据
  */
  registerHandler = (handerName: string, callbackId: string, data: object) => {
    UIManager.dispatchViewManagerCommand(
      this.getWebViewHandle(),
      this.getCommands().registerHandler,
   ...
   * @param data 传递的数据
   */
  callHandler = (handerName: string, callbackId: string, data: object) => {
    UIManager.dispatchViewManagerCommand(
      this.getWebViewHandle(),
      this.getCommands().callHandler,
      [handerName, callbackId, data]
    );
  };
